...
public class MainMemory extends AbstractMainMemory {
  ...
  @Override public byte[] integerToBytes (int i) {
      ...
      resultArray[0] = (byte) ((i >> 24) & 0xFF);
      resultArray[1] = (byte) ((i >> 16) & 0xFF);
      resultArray[2] = (byte) ((i >> 8) & 0xFF);
      resultArray[3] = (byte) (i & 0xFF);
      return resultArray;
  }
  ...
  @Override protected byte[] get (int address, int length) throws InvalidAddressException {
      if (address < 0 || address + length - 1 > length()){
          throw new InvalidAddressException();
      }
      byte[] resultArray = new byte[length];
      for(int i = 0; i < length; i++){
          resultArray[i] = mem[address + i];
      }
    return resultArray;
  }
  ...
  @Override protected void set (int address, byte[] value) throws InvalidAddressException {
      if (address < 0 || address + value.length - 1 > length()){
          throw new InvalidAddressException();
      }
